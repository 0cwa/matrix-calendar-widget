...
export function CalendarCreateDialog({
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    try {
      const calendar = await createCalendar({ name: trimmedName });
      onCreated?.(calendar);
      onClose();
    } catch {
      setError(
        new Error(
          t(
            ...
            'The calendar could not be created. Try again.',
          ),
        ),
      );
    } finally {
      ...
      <form onSubmit={handleSubmit}>
        ...
        <DialogContent>
          <Stack mt={1} spacing={2}>
            ...
            <Alert severity="info">
              {t(
                'calendars.create.eventOnly',
                'This creates an event calendar for VEVENT entries.',
              )}
            </Alert>
          </Stack>
        </DialogContent>
