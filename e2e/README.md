...
1. **Matrix Calendar Bot**: By default, it uses the image that was built by running `yarn docker:build` in the root folder of this repository.
   Building the container at least once is required to run the tests.
   You can build the container by running `yarn build`, followed by `yarn docker:build`. Alternatively, you can set the `BOT_CONTAINER_IMAGE` environment variable to use a custom container image for the Matrix Calendar Bot.
2. **Matrix Calendar Widget**: You can either use the container created using `yarn build` as above, or test against your local copy of the widget.
