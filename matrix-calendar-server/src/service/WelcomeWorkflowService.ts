...
export class WelcomeWorkflowService {
  ...
  public async handleAddWidgetCommand(roomId: string) {
    ...
    if (!widgetAdmin) {
      ...
      this.logger.warn(
        `Can't add Matrix Calendar widget to room, welcome.errors.notEnoughPermissions ${context.originalRoomId}`,
      );
      return;
    }
    ...
    if (exists) {
      ...
      await this.client.sendHtmlText(roomId, html);
      this.logger.warn(
        `Can't add Matrix Calendar widget to room, welcome.errors.meetingWidgetExists ${context.originalRoomId}`,
      );
      return;
    }
    ...
    if (meetingWidgetExists) {
      ...
      if (isWidgetAdmin) {
        htmls.push(
          i18next.t(
            'welcome.botIsAdmin',
            "The bot is a moderator in the room <a href='{{originalRoomLink}}'>{{originalRoomName}}</a>. If you enter the command <code>!meeting setup</code> Matrix Calendar widget will be added to that room.",
            this.i18nOptions(context),
          ),
        );
      } else {
  ...
  private async addWidgetToRoom(roomId: string) {
    ...
    this.logger.verbose(
      `added Matrix Calendar widget to room ${roomId}, stateKey = ${state_key}`,
    );
  }
  ...
  private async inviteUser(
    ...
    const reason = i18next.t(
      ...
      "In this private room bot can help with adding a Matrix Calendar widget to the room '{{originalRoomName}}'.",
